...
export function ReorderButton({ orderId, className }: { orderId: string, className?: string }) {
  ...
  return (
    <button
      ...
      disabled={isPending}
      className={cn("mt-4 sm:mt-0 inline-flex items-center justify-center gap-2 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 disabled:opacity-50 transition-colors", className)}
    >
      {isPending ? (
